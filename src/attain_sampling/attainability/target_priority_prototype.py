...
def main():
    ...
    parser.add_argument("--grid-diagnostic", action="store_true")
    ...
    base = mapping.MappingConfig(
        ...
        sample_period_s=2.0, grid_shape=(12, 8),
        dp_grid_shape=(7, 5) if args.grid_diagnostic else (5, 4),
    ...
    if args.grid_diagnostic:
        calibration = json.loads((ROOT / ".scratch/target-priority-prototype/results/calibration.json").read_text())
        target = calibration["target"]
        calibration["source"] = "original results/calibration.json; reused without recalibration"
        metadata["source_hashes"]["diagnostic_protocol"] = digest(
            ROOT / ".scratch/target-priority-prototype/grid-diagnostic-protocol.md")
        write_json(output / "metadata.json", metadata)
    else:
        result = mapping.run_mapping(base, "adaptive")
        target = 1.05 * result["summary"]["mean_variance"]
        calibration = {"rule": "1.05 * nominal B2 final mean variance", "target": target,
